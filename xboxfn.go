...
// Load proxies in main() or init() function, not at top-level
...
// Send Discord webhook notification for exclusive hits
func sendDiscordWebhookForExclusive(acc, displayName, skinsList, epicEmail, alternateMethods, lastPlayed string, totalVbucks int, hasStw bool) {
...
// Send Discord webhook notification for all hits
...
func calculateAccountQuality(skinCount, vbucks int, hasStw, hasOg, hasRare bool, twofaStatus string) int {
	...
	// Skin count scoring (0-25 points)
	switch {
	case skinCount >= 300:
		score += 25
	case skinCount >= 100:
	...
	// STW access (10 points)
	if hasStw {
		score += 10
	}

	// 2FA protection (10 points)
	if twofaStatus == "true" {
		score += 10
	}

	// Rare skins bonus (0-30 points)
	rareValue := 0
	if hasOg {
		rareValue += 20
	}
...
// Removed redeclaration of Parse. Use the version from variables.go
